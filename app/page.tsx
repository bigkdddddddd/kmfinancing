...
import { CallbackSection, Footer, TopNav } from "./components";
...
function LoanFinder() {
  return <section className="finder" id="finder">
    <div className="finder-copy">
      ...
      <h2>Choose the finance pathway that fits your next move.</h2>
      <p>Tap a tab to open a dedicated page for that finance type, then request a tailored estimate from the callback form at the bottom.</p>
    </div>
    <div className="finder-box">
      <div className="tabbar">{finderTabs.map((item, index) => <Link key={item.href} href={item.href} className={index === 0 ? "active" : ""}>{item.label}</Link>)}</div>
      <div className="fieldrow">
        <label><span>Purchase price</span><input placeholder="$" /></label>
        <label><span>Deposit / equity</span><input placeholder="$" /></label>
        <label><span>Annual income</span><input placeholder="$" /></label>
      </div>
      <div className="finder-footer"><div><strong>Home loan</strong><p>Estimated only. Final options depend on lender policy and assessment.</p></div><Link href="/home-loans" className="btn">Open page</Link></div>
    </div>
  </section>;
}
...
function Calculators() {
  ...
  return <div className="calc"><div className="switch"><button onClick={() => setMode("home")} className={mode === "home" ? "on" : ""}>Borrowing Power</button><button onClick={() => setMode("car")} className={mode === "car" ? "on" : ""}>Car Loan</button><button onClick={() => setMode("repay")} className={mode === "repay" ? "on" : ""}>Repayments</button></div><div className="panel">
    ...
    <label>Rate assumption<input type="number" step="0.01" value={rate} onChange={e => setRate(+e.target.value)} /></label><div className="result"><span>{result[0]}</span><strong>{result[1]}</strong><p>For a precise figure, request a callback.</p></div></div></div>;
}
...
function Eligibility() {
  ...
  if (done) return <div className="wizard"><h2>Your estimate is ready</h2><p className="big">{answers["Finance type"] === "Car loan" ? "$35,000 - $60,000" : "$450,000 - $750,000"}</p><div className="notice">Your estimate is a helpful starting point. For a precise figure based on lender policy, request a callback with KM Financing.</div><Link href="#callback" className="btn">Request Callback</Link></div>;
